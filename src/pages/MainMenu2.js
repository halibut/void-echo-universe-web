...
import SongData, { SongList } from "../service/SongData";
...
const NUM_TRACKS = SongList.length;

const TrackItem = ({songData, selected, position}) => {
  const color = selected ? '#bfd1ff' : "#ddd"; 

  const ratio = position / (NUM_TRACKS);
  const topPercent = `${100 * ratio}%`;
  const animDelay = `${1000 * ratio}ms`

  return (
    <Link className='track-item fade-in' style={{/*top: topPercent,*/ animationDuration:"1000ms"}} path={Utils.trackNameToPath(songData.title)} navOptions={{time:3000}}>
...
const MainMenu2 = ({isLoadingOut, isLoadingIn, fullyLoaded}) => {
  ...
  return (
    <div className='center' style={{position:"relative", flex:1, width:'100%', height:'100%', alignItems:'center', justifyContent:'center', }}>
